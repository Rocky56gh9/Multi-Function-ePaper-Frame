#!/bin/bash

LOGFILE="/var/log/setup_project.log"

log_and_print() {
    echo "$1"
    echo "$1" >> "$LOGFILE"
}

retry_command() {
    local n=1
    local max=5
    local delay=5

    while true; do
        "$@" && break || {
            if [[ $n -lt $max ]]; then
                ((n++))
                log_and_print "Command failed. Attempt $n/$max:"
                sleep $delay
                delay=$((delay * 2))  # Exponential backoff
            else
                log_and_print "The command has failed after $n attempts."
                return 1
            fi
        }
    done
}

check_network() {
    wget -q --spider http://google.com
    if [ $? -eq 0 ]; then
        log_and_print "Network is up"
        return 0
    else
        log_and_print "Network is down"
        return 1
    fi
}

manual_install_package() {
    local package_name="$1"
    local package_url="$2"

    log_and_print "Manually downloading and installing $package_name..."
    wget $package_url -O ${package_name}.tar.gz
    tar -xzf ${package_name}.tar.gz
    pip3 install --no-cache-dir ${package_name}*
}

install_package() {
    local package_name="$1"
    local pip_command="$2"
    local manual_url="$3"

    log_and_print "Attempting to install $package_name..."
    if ! retry_command $pip_command; then
        log_and_print "$package_name installation failed via pip. Attempting manual installation..."
        if ! manual_install_package "$package_name" "$manual_url"; then
            log_and_print "$package_name manual installation also failed."
            return 1
        fi
    fi
    return 0
}

clone_repo() {
    local repo_url="$1"
    local repo_dir="$2"
    local zip_url="${repo_url}/archive/main.zip"

    if retry_command git clone $repo_url $repo_dir; then
        return 0
    fi

    if [ -f "$HOME/.ssh/id_rsa" ]; then
        local ssh_url=${repo_url/https:\/\/github.com\//git@github.com:}
        log_and_print "Trying to clone using SSH: $ssh_url"
        if retry_command git clone $ssh_url $repo_dir; then
            return 0
        fi
    fi

    log_and_print "Trying to download and unzip: $zip_url"
    if retry_command wget $zip_url -O ${repo_dir}.zip; then
        unzip ${repo_dir}.zip
        mv ${repo_dir}-main $repo_dir
        return 0
    fi

    log_and_print "Failed to clone the repository."
    return 1
}

is_installed() {
    dpkg -l | grep -qw "$1"
}

export PATH=$PATH:$HOME/.local/bin

if ! check_network; then
    log_and_print "Network check failed. Please ensure you are connected to the internet."
    exit 1
fi

log_and_print "Updating system packages..."
retry_command sudo apt-get update --fix-missing

essential_packages=(git git-lfs libjpeg-dev libopenjp2-7 python3-pip wget unzip)
for package in "${essential_packages[@]}"; do
    if is_installed $package; then
        log_and_print "$package is already installed."
    else
        log_and_print "Installing $package..."
        retry_command sudo apt-get install -y $package
    fi
done

git config --global http.postBuffer 524288000
git lfs install

clone_repo "https://github.com/Rocky56gh9/multimode-epaper-frame.git" "multimode-epaper-frame"
if [ ! -d "multimode-epaper-frame" ]; then
    log_and_print "Failed to clone the repository. Exiting."
    exit 1
fi

cd multimode-epaper-frame || exit

python_packages=(
    "Pillow https://files.pythonhosted.org/packages/58/b7/ece20939f84f3a4f9d6b344df82d30e0ed4b35a5e58e9a4b7b15c7870c2b/Pillow-8.1.2.tar.gz"
    "pytz https://files.pythonhosted.org/packages/41/aa/0b509ee60282b11d6a09c218beeb24c8f2281a2e8d7b708d9d44bb2dfdeb/pytz-2024.1.tar.gz"
    "beautifulsoup4 https://files.pythonhosted.org/packages/91/f7/5e1a6e20b7edc17219f3fd1c10fc8c1708a80c867b09f2e2f5aaf8d03b65/beautifulsoup4-4.12.3.tar.gz"
    "praw https://files.pythonhosted.org/packages/2d/49/1f8ea5e875cf1a31c4b9d4f0e293c1e2c64c98a0f85ed19fd0f0c4f4ff8e/praw-7.7.1.tar.gz"
    "python-crontab https://files.pythonhosted.org/packages/fb/35/5a63ea0ed7c91f2a0c71e62a7f85edff6ef0efb99f8954781a3429cbfb69/python-crontab-2.5.1.tar.gz"
    "RPi.GPIO https://files.pythonhosted.org/packages/fd/57/3a2a4b1dc42b55c01e2b82ddda12e3b0e7ecb9ffb9f1c54e4785e89a6f6b/RPi.GPIO-0.7.1.tar.gz"
    "spidev https://files.pythonhosted.org/packages/6b/2e/60a5e29b8e1cb8d7e6b8cfc8a1251156a2b8f5b8c6cbe5cbdf979117f143/spidev-3.5.tar.gz"
    "timezonefinder https://files.pythonhosted.org/packages/2b/f7/10e278b8ef145da2e7f1480d7180b296ec53535985dc3bc5844f7191d9a0/timezonefinder-6.5.0.tar.gz"
)

failed_packages=()
for pkg in "${python_packages[@]}"; do
    IFS=" " read -r name url <<< "$pkg"
    install_package "$name" "pip3 install --no-cache-dir $name" "$url" || failed_packages+=("$name")
done

if [ ${#failed_packages[@]} -ne 0 ]; then
    log_and_print "The following packages failed to install:"
    for pkg in "${failed_packages[@]}"; do
        log_and_print "- $pkg"
    done
else
    log_and_print "All packages installed successfully."
fi

clone_repo "https://github.com/waveshare/e-Paper.git" "e-Paper"
if [ ! -d "e-Paper" ]; then
    log_and_print "Failed to clone the e-Paper repository. Exiting."
    exit 1
fi

log_and_print "Enabling SPI interface..."
retry_command sudo raspi-config nonint do_spi 0

log_and_print "Configuring device for USB access..."
sudo modprobe libcomposite
sudo mkdir -p /sys/kernel/config/usb_gadget/g1
cd /sys/kernel/config/usb_gadget/g1 || exit
echo 0x1d6b | sudo tee idVendor # Linux Foundation
echo 0x0104 | sudo tee idProduct # Multifunction Composite Gadget
echo 0x0100 | sudo tee bcdDevice # v1.0.0
echo 0x0200 | sudo tee bcdUSB # USB2

sudo mkdir -p strings/0x409
echo "fedcba9876543210" | sudo tee strings/0x409/serialnumber
echo "Manufacturer" | sudo tee strings/0x409/manufacturer
echo "Product" | sudo tee strings/0x409/product

sudo mkdir -p configs/c.1/strings/0x409
echo "Config 1: ECM network" | sudo tee configs/c.1/strings/0x409/configuration
echo 250 | sudo tee configs/c.1/MaxPower

sudo mkdir -p functions/ecm.usb0
echo "DE:AD:BE:EF:00:01" | sudo tee functions/ecm.usb0/host_addr || log_and_print "Skipping host_addr configuration as it is busy."
echo "DE:AD:BE:EF:00:02" | sudo tee functions/ecm.usb0/dev_addr || log_and_print "Skipping dev_addr configuration as it is busy."

if [ ! -L configs/c.1/ecm.usb0 ]; then
    sudo ln -s functions/ecm.usb0 configs/c.1/
else
    log_and_print "Symbolic link 'configs/c.1/ecm.usb0' already exists. Skipping link creation."
fi

cd "$HOME/multimode-epaper-frame" || exit

log_and_print "Making Python scripts executable..."
chmod +x scripts/*.py

log_and_print "Running configuration scripts..."
retry_command python3 config/dadjokes_showerthoughts_config.py
retry_command python3 config/weatherstation_config.py
retry_command python3 config/crontab_config.py

log_and_print "Setup complete. Please reboot your system to apply all changes."
